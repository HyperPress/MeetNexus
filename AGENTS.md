...
- `main` 是受保护的稳定分支，只有组长可以合并进入该分支；任何成员和 AI 都不得直接向 `main` 推送开发提交。
- 开始任务前必须先同步 `main`，再创建自己的功能分支，命名为 `feat/<简短任务名>`、`fix/<简短问题名>` 或 `docs/<简短主题>`。
- 一个分支只处理一项明确任务。完成后推送分支并创建 Pull Request，由组长审核、解决冲突并统一合并到 `main`。
- 合并后删除远端功能分支；本地分支可按成员需要保留或删除。
