...
> 当前项目处于基础工程阶段，已完成技术路线、目录分层、本机服务准备和质量规范；
> 房间、参会者、媒体鉴权代理及多人音视频闭环仍在开发中。当前暂无公开在线演示。
...
- PostgreSQL、Redis 与 Live777 的服务边界和本机运行方式。
- WHIP/WHEP 媒体请求经 Axum 鉴权代理的架构方案。
- OpenAPI 契约优先、结构化日志、静态检查和端到端测试规范。
- 中文项目文档、团队协作规则和本机初始化脚本。

### 开发中

- 房间与参会者管理。
- API 统一响应、配置加载和依赖健康检查。
