...
- API 已实现 `/health`（进程存活）和 `/ready`（PostgreSQL、Redis、Live777 就绪）以及房间创建、查询、加入、离开、心跳接口，并提供 PostgreSQL/Redis 适配器。成员退出记录 `left_at`，会议不会因暂时无人而关闭，因此可用会议号再次加入；历史成员与录制关联仍保留。
...
- 离会接口幂等执行数据库逻辑离会；媒体会话关闭允许已离会成员继续凭原绑定令牌回收自己的 Live777 会话，避免先离会后清理媒体失败。
- 房间页显式离开时等待退出接口成功后返回首页；站内导航、浏览器前进后退时使用带 Bearer 令牌的 `fetch(..., { keepalive: true })` 尽力完成退出，并立即清理标签页级成员会话。刷新页面会保留标签页级成员会话，以便恢复媒体控制。
