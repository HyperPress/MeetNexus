...
- API 已实现 `/health`（进程存活）和 `/ready`（PostgreSQL、Redis、Live777 就绪）以及房间创建、查询、加入、离开、心跳接口，并提供 PostgreSQL/Redis 适配器。成员退出记录 `left_at`，最后一名有效成员退出时记录房间 `closed_at`；已关闭房间不再允许查询或加入，历史成员与录制关联仍保留。
...
- API 已提供受保护的房间 WebSocket 事件：浏览器用 `meetnexus.<JWT>` 子协议建立连接，成员加入或离开、主媒体开始或停止、屏幕共享开始或停止、文字聊天和举手状态变化时立即广播。新连接会收到当前媒体发布者、最近 100 条聊天消息和举手成员快照；聊天和举手仅保存在 API 房间事件内存中，房间关闭或 API 重启后清空。客户端互动命令由服务端按成员会话确定身份并校验，禁止客户端指定消息发送者。
- 屏幕共享已使用独立于摄像头/麦克风的 WHIP/WHEP 流发布与回收，避免覆盖主媒体流；房间事件通知其他成员自动订阅并展示远端屏幕流。仅发布者关闭屏幕 WHIP 会话会广播停止事件，订阅者退出不会影响其他成员观看。
- API 已实现主持人控制的逐成员主媒体录制任务及 PostgreSQL 元数据；Live777 产生 DASH MPD 和分片。会议页已提供主持人逐成员开始/停止录制、状态查看和回放控制；浏览器使用原生 MediaSource 经成员令牌保护的接口读取已停止录制的清单及同目录 `.m4s` 分片，回放文件仅从 `RECORDING_STORAGE_ROOT` 读取，并拒绝目录穿越和直接文件系统访问。
...
- 离会接口幂等执行数据库逻辑离会和空房关闭；媒体会话关闭允许已离会成员继续凭原绑定令牌回收自己的 Live777 会话，避免先离会后清理媒体失败。
- 房间页显式离开时等待退出接口成功后返回首页；站内导航、浏览器前进后退、刷新和关闭标签页时使用带 Bearer 令牌的 `fetch(..., { keepalive: true })` 尽力完成退出，并立即清理标签页级成员会话。
