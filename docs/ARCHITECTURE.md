...
- API 已提供受保护的房间 WebSocket 事件：浏览器用 `meetnexus.<JWT>` 子协议建立连接，成员加入或离开时立即广播；事件积压时发送重新同步提示，前端据此重新读取房间信息。
- 屏幕共享已使用独立于摄像头/麦克风的 WHIP/WHEP 流发布与回收，避免覆盖主媒体流；房间事件通知其他成员自动订阅并展示远端屏幕流。
- 当前没有录制或回放实现。
