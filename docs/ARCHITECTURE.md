...
- API 已实现主持人控制的逐成员主媒体录制任务及 PostgreSQL 元数据；Live777 产生 DASH MPD 和分片。已提供成员令牌保护的回放文件接口，仅从 `RECORDING_STORAGE_ROOT` 内读取已停止录制的清单及同目录 `.m4s` 分片，并拒绝目录穿越和直接文件系统访问；浏览器 DASH 播放器尚未接入。
