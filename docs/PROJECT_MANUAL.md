...
当前已完成工程基础、后端房间模块、中文房间入口和浏览器本地设备预览。前端尚未接入房间 API；用户身份、WebSocket、WHIP/WHEP 媒体代理、多人音视频、录制和回放尚未实现。

下一步按以下顺序实施：

1. 在开发机创建 `meetnexus` PostgreSQL 用户与数据库并执行 SQLx 迁移。
2. 为前端增加 Zod 响应模型和 HTTP 客户端，接入真实房间接口。
3. 补充用户身份、房间权限和依赖就绪检查。
4. 完成媒体授权设计并接入 WHIP/WHEP。
5. 实现最小双人音视频闭环。
