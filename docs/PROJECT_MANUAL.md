...
当前已完成工程基础、房间 PostgreSQL/Redis 模块、中文房间入口、会前设备预览、房间成员 JWT 鉴权、WebSocket 实时事件、同源 WHIP/WHEP 媒体代理、基础多人音视频闭环及 Live777 逐成员录制任务。会议页已接入主持人录制控制、录制状态和原生 MediaSource DASH 回放；回放文件仅对会议成员开放。

下一步按以下顺序实施：

1. 在具备实体摄像头、麦克风和扬声器的两台浏览器上完成手动音视频验收。
2. 根据实际部署需要补充录制存储保留策略与容量监控。

## 8. 实体设备人工验收清单

自动化测试使用隔离的 Chromium 媒体设备和 WebRTC 实现，不能替代实体摄像头、麦克风、扬声器及系统屏幕选择器的验收。交付前请在两台具有真实设备的电脑或两个独立浏览器配置文件中完成以下检查，并记录日期、浏览器版本和结果：

1. 访问 `http://127.0.0.1:5173`，确认 `http://127.0.0.1:8080/ready` 返回 `200`，且 PostgreSQL、Redis、Live777 三项依赖均为 `true`。
2. 甲创建会议、乙使用会议号加入；两人均允许浏览器访问摄像头和麦克风后，点击“启动音视频设备”。双方应看到对方画面并听到对方声音，连接状态显示“已连接”。
3. 在甲侧分别点击“关闭摄像头”和“关闭麦克风”，确认甲侧预览和乙侧对应的远端视频或音频停止；重新打开后应恢复，无需重新加入会议。
4. 让甲侧网络短暂断开后恢复，或在浏览器开发者工具中模拟离线再恢复；甲侧应显示恢复中的连接状态，并在恢复后重新发布媒体，乙侧能继续收到音视频。
5. 由甲发起屏幕共享并选择一个真实窗口或屏幕，确认乙侧能看到共享画面；结束共享后，乙侧共享画面应消失，主音视频通话保持可用。
6. 由主持人为一名正在发布主媒体的成员开始录制，持续至少三秒后停止。刷新录制列表后点击“播放回放”，确认音视频可以在浏览器中播放；非该会议成员不得取得回放文件。
7. 两人主动离开会议或关闭页面，确认浏览器设备指示器熄灭、远端画面被清理，API `/ready` 仍返回 `200`。

若任一项失败，请保留浏览器控制台信息、API JSON 日志中的 `request_id`，以及复现步骤后再提交缺陷；不得将 JWT、数据库密码或 Live777 Token 写入记录。
