...
- 完成 Linux ECS 直接部署：PostgreSQL、Redis、Live777、Rust API 与 Nginx 均作为 systemd/主机服务运行；Nginx 提供公网 HTTPS、同源 API 与 WebSocket，API 和 Live777 保持回环监听。部署配置与运维检查见 `docs/ECS_DEPLOY.md`。

## 进行中

- 等待在具备实体摄像头、麦克风和扬声器的两台浏览器上完成手动音视频验收；自动化与隔离 Chromium 媒体验收已完成。
- 等待在 ECS 云防火墙放行 UDP `32768/60999` 后，以两台不同网络的真实设备完成公网音视频验收；若失败，再基于浏览器 WebRTC 日志评估 TURN。
...
- 2026-08-04：完成 Linux ECS 直接部署与公网 HTTPS 验证：前端首页、`/health`、`/ready` 均可经 HTTPS 从公网访问，HTTP 会重定向至 HTTPS；PostgreSQL、Redis、Live777、API 与 Nginx 服务均为 active。Let's Encrypt IP 证书已配置自动续期。尚未进行两台真实设备的跨网 WebRTC 验收，需先放行 Live777 使用的 UDP `32768/60999`。
