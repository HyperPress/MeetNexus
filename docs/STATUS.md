...
- 2026-08-04：完成 Windows 本机 Caddy 部署入口。新增 Caddyfile、启动脚本、当前用户根证书导入脚本和仅限专用网络的防火墙脚本；前端生产构建、Memurai、Live777、API 与 Caddy 已在本机联通验证，`https://localhost/`、`/health`、`/ready` 均返回 200，HTTP 会重定向到 HTTPS。API 继续仅监听 `127.0.0.1:8080`。局域网其他设备尚需信任 Caddy 本地根证书并由管理员执行防火墙放行脚本；真实双设备音视频验收仍待执行。
- 2026-08-04：完成 ngrok 临时外网入口验证。ngrok HTTPS 隧道已转发到本机 Caddy 的 `https://localhost:443`，公网首页、`/health` 和 `/ready` 均返回 200；新增启动脚本，确保仅向 ngrok 子进程移除 HTTP(S) 代理变量以规避免费账户 `ERR_NGROK_9009`。该方式不代理 Live777 的 WebRTC UDP，外网真实音视频仍需分别验收并按网络情况配置 UDP/TURN。
